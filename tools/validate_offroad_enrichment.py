"""בדיקות השלמת מטא-דאטה Off-Road — גרסת מסמך 2.2.3; גרסת מוצר 2.4.0."""
...
def main() -> int:
    ...
    check(routes["documentVersion"] == DATA_DOCUMENT_VERSION, "routes dataset document version is 2.1.8")
    check(metadata["documentVersion"] == "2.1.7", "metadata dataset document version is 2.1.7")
    ...
    check(metadata_js.startswith("/* מטא-דאטה Off-Road — גרסת מסמך 2.1.7"), "metadata JavaScript shows document version")
    ...
    check(f"גרסת מסמך {SITE_DOCUMENT_VERSION}" in index, "main HTML displays document version 2.2.3")
    ...
    check(manifest["document_version"] == SITE_DOCUMENT_VERSION, "manifest document version is 2.2.3")
