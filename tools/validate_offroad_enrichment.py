"""בדיקות השלמת מטא-דאטה Off-Road — גרסת מסמך 2.2.0; גרסת מוצר 2.2.0."""
...
def main() -> int:
    ...
    check(f"גרסת מסמך {APP_DOCUMENT_VERSION}" in index, "main HTML displays document version 2.2.0")
    ...
    check("offroad-all-metadata.js" in service_worker and APP_DOCUMENT_VERSION in service_worker, "service worker caches metadata with the 2.2.0 cache version")
    check(manifest["document_version"] == APP_DOCUMENT_VERSION, "manifest document version is 2.2.0")
