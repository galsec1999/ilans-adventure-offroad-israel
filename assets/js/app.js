/* ספר מסלולי אדוונצ׳ר ואופרוד — גרסת מסמך 2.2.0; גרסת מוצר 2.2.0 */
(() => {
  'use strict';

  const PRODUCT_VERSION = '2.2.0';
  const DOC_VERSION = '2.2.0';
  ...
  async function exportTripHtml(card) {
    ...
    const localNotesHtml = await window.R123FieldNotes?.routeExportHtml?.(data.id) || '';
