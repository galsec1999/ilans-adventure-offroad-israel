...
<body><main class="page"><header class="hero"><div class="version">גרסת מוצר ${PRODUCT_VERSION} · גרסת מסמך ${DOC_VERSION}</div><h1>${escapeHtml(data.title)}</h1><p>${escapeHtml(data.region)} · ${escapeHtml(data.difficultySource)} · ${escapeHtml(data.distanceDuration)}</p><button class="print-action" type="button" onclick="window.print()">הדפסה / שמירה כ־PDF</button></header><section class="export-overview"><h2>כל פרטי הכרטיס</h2><div class="meta">${metaRows}</div></section><div class="route-body">${clone.innerHTML}</div>${sourceSection}${mapSection}${localNotesHtml}<section class="notice"><h2>אחריות ובטיחות</h2><p>מסמך זה מרכז מידע קיים ואינו אישור שהמסלול פתוח, חוקי, עביר, בטוח או מתאים לקבוצה. המוביל והקבוצה חייבים לבדוק סמוך ליציאה מזג אוויר, שיטפונות, שטחי אש, ביטחון, שמורות, שערים, מצב השטח ורמת הרוכבים. קושי הוא סובייקטיבי.</p></section><footer class="footer"><b>כרטיס טיול עצמאי · גרסת מוצר ${PRODUCT_VERSION} · גרסת מסמך ${DOC_VERSION}</b><p>מקור הכרטיס בספר: <a href="${escapeHtml(data.cardUrl)}">${escapeHtml(data.cardUrl)}</a></p><p>התמונות הוטמעו בקובץ. מפות Off‑Road דורשות חיבור לאינטרנט.</p></footer></main></body></html>`;
